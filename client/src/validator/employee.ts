...
export const EmployeeRegister = EmployeeSubmit.extend({
  name: z.string({ error: 'name is required' }).min(4, 'name must have 4 or more chars').max(30, 'name must be under 30 chars')
    .regex(/^[a-zA-Z0-9]+(?: [a-zA-Z0-9]+)*$/, 'invalid employee name'),
  profilePicture: z.instanceof(FileList)
    .refine(files => files.length == 1, "Single profile picture is required")
    .refine(files => files[0]?.size <= 2 ** 20, `Max image size is 1MB.`)
    .refine(
